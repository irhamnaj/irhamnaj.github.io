...
A small startup is able to target users based on simple rules predefined by marketing such as sorting customers by place of residence. But, for companies with sizes as same as Tokopedia and Bukalapak with million daily customers and transaction. they need to use more advance method to target their marketing.

One such approach is Cluster Analysis, Use mathematical algorthm to discover groups of similar customers based on many criteria such as customer daily habit, demographic, economic condition, and other characteristics.
...
there are numerous analyzes and visualization that can be found in the data, but we can't show that all because it depends on what we the retail need or their KPI.

assume the retail wants to improve monthly revenue through some marketing action using customer characteristics information. So we can use Transaction KPI and Customer KPI as main analysis to find information before modeling segmentation
...
### Generate Dataframe with Transaction's KPI
```python

#generate monlty revenue by aggergating revenue
monthly_kpi = pd.DataFrame(new_df.groupby([new_df.year,new_df.month])['Revenue'].sum())

#montly revenue growth
monthly_kpi['MonthlyGrowth'] = monthly_kpi['Revenue'].pct_change()

#monthly active customer
monthly_kpi['ActiveCustomer'] = pd.DataFrame(new_df.groupby([new_df.year,new_df.month])['CustomerID'].nunique())

#total order
monthly_kpi['TotalOrder'] = pd.DataFrame(new_df.groupby([new_df.year,new_df.month])['Quantity'].sum())

#average revenue per order
monthly_kpi['AverageRevenuePerOrder'] = pd.DataFrame(round(new_df.groupby([new_df.year,new_df.month])['Revenue'].mean()))

#show data
monthly_kpi.index.set_names(['Year', 'Month'], inplace = True)
monthly_kpi
```

|            |          Revenue |   MonthlyGrowth |   ActiveCustomer |   TotalOrder |   AverageRevenuePerOrder |
|:-----------|-----------------:|----------------:|-----------------:|-------------:|-------------------------:|
| (2010, 12) | 572714           |    nan          |              885 |       312280 |                       22 |
| (2011, 1)  | 569445           |     -0.00570765 |              741 |       349147 |                       27 |
| (2011, 2)  | 447137           |     -0.214784   |              758 |       265638 |                       22 |
| (2011, 3)  | 595501           |      0.331807   |              974 |       348544 |                       22 |
| (2011, 4)  | 469200           |     -0.212091   |              856 |       292225 |                       21 |
| (2011, 5)  | 678595           |      0.446279   |             1056 |       373685 |                       24 |
| (2011, 6)  | 661214           |     -0.025613   |              991 |       363699 |                       24 |
| (2011, 7)  | 600091           |     -0.0924401  |              949 |       369432 |                       22 |
| (2011, 8)  | 645344           |      0.07541    |              935 |       398938 |                       24 |
| (2011, 9)  | 952838           |      0.476482   |             1266 |       544899 |                       24 |
| (2011, 10) |      1.03932e+06 |      0.0907608  |             1364 |       593908 |                       21 |
| (2011, 11) |      1.16182e+06 |      0.117864   |             1665 |       681888 |                       18 |
| (2011, 12) | 518193           |     -0.553981   |              615 |       287413 |                       30 |

### Monthly Revenue

```python
monthly_kpi.plot.line(y='Revenue',
...
![image](https://irhamnaj.github.io/assets/images/monthly revenue.png)

As we can see there was some significant drop in february, may, juny and july with retail's revenue start to growing at august 2011. i will not analysis negative drop in december 2011, because it was too high and need analysis to find the cause of that pattern.

### Monthly Growth

```python
monthly_kpi.plot.line(y='MonthlyGrowth',
                      ...
                      markerfacecolor='white')
```
![image](https://irhamnaj.github.io/assets/images/monthly_growth.png)

The line graph illustrates percent growt of the retail. Overall there was negative drop in some month with the biggest negative drop was happend in april 2011. Suprisingly, percent gowth between september - oktober 2011 showing negative drop although it looks increase in revenue line chart.

### Total order per Month

```python
monthly_kpi.plot.line(y='TotalOrder',
                      ...
                      markerfacecolor='white')
```
![image](https://irhamnaj.github.io/assets/images/order_per_month.png)

total order showing same pattern with two line chart before: there was significant drop during april 2011.

### Average Revenue Per order per month

```python
monthly_kpi.plot.line(y='AverageRevenuePerOrder',
                      title = 'Average Revenue Per order per month',
                      color = 'g',
                      marker='h', 
                      markersize=10, linewidth=2,
                      markerfacecolor='white')
```
![image](https://irhamnaj.github.io/assets/images/avg_order_per_month.png)

line chart above depicts average revenue per month from retail, we can see that there was significant drop during february until april. surprisingly, average revenue started to rise very sharply in December.

### max date and total customer for every year month

all line charts above show abnormal pattern in december (example: decrease sharply in total order but rise very sharply in average revenue). So i will check data from december (start from total customer)

```python
list_max_date = []

for x in sorted(new_df.yearmonth.unique()):
  df_month = new_df[new_df.yearmonth == x]
  max_date = df_month.InvoiceDate.max().strftime('%Y-%m-%d')
  min_date = df_month.InvoiceDate.min().strftime('%Y-%m-%d')
  count_data = df_month['CustomerID'].nunique()
  date_diff = (df_month.InvoiceDate.max() - df_month.InvoiceDate.min()).days

  list_max_date.append([x, min_date, max_date, date_diff, count_data])

df_date = pd.DataFrame(list_max_date, columns=['date month','min date',
                                               'max date', 'date diff',
                                               'count data']).set_index('date month')
df_date
```

|   date month | min date   | max date   |   date diff |   count data |
|-------------:|:-----------|:-----------|------------:|-------------:|
|       201012 | 2010-12-01 | 2010-12-23 |          22 |          885 |
|       201101 | 2011-01-04 | 2011-01-31 |          27 |          741 |
|       201102 | 2011-02-01 | 2011-02-28 |          27 |          758 |
|       201103 | 2011-03-01 | 2011-03-31 |          30 |          974 |
|       201104 | 2011-04-01 | 2011-04-28 |          27 |          856 |
|       201105 | 2011-05-01 | 2011-05-31 |          30 |         1056 |
|       201106 | 2011-06-01 | 2011-06-30 |          29 |          991 |
|       201107 | 2011-07-01 | 2011-07-31 |          30 |          949 |
|       201108 | 2011-08-01 | 2011-08-31 |          30 |          935 |
|       201109 | 2011-09-01 | 2011-09-30 |          29 |         1266 |
|       201110 | 2011-10-02 | 2011-10-31 |          29 |         1364 |
|       201111 | 2011-11-01 | 2011-11-30 |          29 |         1665 |
|       201112 | 2011-12-01 | 2011-12-09 |           8 |          615 |

Oevrall, in December 2011, we only have data from 1 December to 9 December which is different from data from other months which on average have data until the end of the month (can be seen in the date difference column). In addition, December 2011 also had the least number of customers. Looking at the two facts above, it is not surprising that through the line chart, data for December 2011 has a very different pattern from data from other months.

### Total Revenue in December 2011

```python
data_rev_dec = pd.DataFrame(new_df[new_df.yearmonth == 201112].groupby('CustomerID')['Revenue'].sum())
data_rev_dec.plot.scatter(x = 'Revenue',y = 'Revenue')
```
![image](https://irhamnaj.github.io/assets/images/revenue_december.png)

In the scatter plot above, you can see that there are outliers in the data for December 2011, this data is what causes average revenue in December 2011 to have a very extreme pattern of increase, even though the number of customers and invoices has the least amount

considering the balance of data in terms of date distance and the absence of supporting information to maintain the December 2011 data. Then for further analysis the data from December 2011 will be deleted
...
data_rev_dec.plot.scatter(x = 'Revenue',y = 'Revenue')
```
![image](https://irhamnaj.github.io/assets/images/total_active_customer.png)

same as transaction KPI, significant drop was happends in april 2011.

### New vs Existing Customers

```python
#create dataframe contains customer and first time they buy
first_purchase = new_df.groupby('CustomerID').InvoiceDate.min().reset_index()
first_purchase.columns = ['CustomerID', 'CustomerFirstPurchase']

#create columns yearmonth
first_purchase['FirstPurchaseYearMonth'] = first_purchase['CustomerFirstPurchase'].map(lambda x: 100*x.year + x.month)

#merge the data
new_df = pd.merge(new_df, first_purchase, on='CustomerID')
...
#if order_purchase_timestamp > min_purchase_date return existing
new_df.loc[new_df['InvoiceDate'] > new_df['CustomerFirstPurchase'],'UserType'] = 'Existing'

new_df[['CustomerID', 'UserType']].head()
```
...
customers_kpi = pd.DataFrame(new_df.groupby([new_df.year,new_df.month, new_df.UserType])['Revenue'].sum())
customers_kpi.index.set_names(['Year', 'Month', 'UserType'], inplace = True)

#plot data
customers_kpi.unstack().plot.line(y='Revenue', 
                                  title='New vs Existing CUstomer Monthly Revenue',
                                  rot=0,
                                  marker='h', 
                                  markersize=10, linewidth=2,
                                  markerfacecolor='white');
...
![image](https://irhamnaj.github.io/assets/images/new_vs_existing.png)

- there was significant growth in existing customers. which is mean our retail has the power to maintain customers always buy products in Retail.
- In the other hand, significant drop happen in new customers. this pattern depicts that 'Retail' failed to get new customers
...
```python
# Recency
# To calculate recency, we need to find out most recent purchase date of each customer 
# and calculate how many day they inactive for (recent purchase date - other purchase date by that customer)
...
#more low our recency, more good for our retail, 
#so i will groupby data by customerID and choose min of recency value
...
Normally, we can perform clustering analysis in the RFM table without using kmeans because the RFM Table generally only consists of 3 tables. For example, making clustering of each column based on the top or bottom order and then combining each clustering into a new cluster based on agreement and analysis between data scientists and related parties. But we can use unsupervised machine learning algorithm like K-means as alternative way. The goal is to open up new perspectives on the most efficient way to get the best number of clusters and unlock information that cannot be obtained when using the quartile method. in this case, I will use K-means as the basis for getting the appropriate clusters.
